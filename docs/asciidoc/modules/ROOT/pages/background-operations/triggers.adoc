...
Generally, it is recommended to use `afterAsync` phase, to prevent some annoying transaction locks.
This will stop the query from remaining pending indefinitely.
...
|===
| Phase | Description
| before | The trigger will be activated right `before` the commit. If no phase is specified, it's the default.
...
| afterAsync | The trigger will be activated right `after` the commit and inside a new transaction and thread that will not
