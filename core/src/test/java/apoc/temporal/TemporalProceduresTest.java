...
public class TemporalProceduresTest
{
    ...
    public void shouldFormatDurationGreaterThanADayWithIsoFormat() {
        ...
        assertEquals("P0002Y1M1DT1H1M1.000S", output);
    }
    
    @Test
    public void shouldFormatDurationGreaterThanADayWithDurationNegative() {
        try {
            TestUtil.testCall(db, "WITH duration.between( datetime(\"2019-07-03T19:41:33\") , datetime(\"2017-06-02T18:40:32\")) AS duration\n" +
                    "RETURN apoc.temporal.formatDuration(duration, 'iso_format', true) AS value", r -> fail());
        } catch (RuntimeException e) {
            assertTrue(e.getMessage().contains("The duration must be positive"));
        }
    }
