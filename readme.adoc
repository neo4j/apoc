...
====
// tag::warnings[]
APOC contains over 450 functions and procedures.
It is *not* recommended to load all of these into the dbms, but instead use the principle of least privilege.
This principle dictates, that you should only load the functions and procedures that you strictly need to execute your queries.
...
As an example, to load `apoc.math.maxInt` and all functions in the `apoc.rel` package, use `+dbms.security.procedures.allowlist=apoc.math.maxInt,apoc.rel.*+`.

For security reasons, procedures that use internal APIs are disabled by default.
Also in this case, it is recommended to use the principle of least privilege and only unrestrict those procedures which you are certain to make use of.
The procedures can be unrestricted using the Neo4j configuration setting `+dbms.security.procedures.unrestricted+` in `$NEO4J_HOME/conf/neo4j.conf` e.g. `+dbms.security.procedures.unrestricted=apoc.cypher.runFirstColumn,apoc.cypher.doIt+`.

If you want to unrestrict a whole package of APOC functions and procedures when using the Neo4j Docker container, you need to amend e.g. `+-e NEO4J_dbms_security_procedures_unrestricted=apoc.cypher.\\\*+` to your `docker run ...` command.
