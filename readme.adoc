...
:apoc-release:2026.10.0
:neo4j-version:2026.10.0
...
cd apoc
./gradlew shadow
cp core/build/libs/apoc-<version>-core.jar $NEO4J_HOME/plugins/
...
You can either copy the jar (core/build/libs) into the neo4j target folder (target/neo4j/plugins folder) or launch it in a
